...
def _strip_leading_date(text):
    """确定性兜底（2026-09-14）：不以具体日期开头。把开头的『2026年9月14日，』『9月14日，』等去掉，直接进正文。"""
    if not text:
        return text
    return re.sub(r"^\s*(?:\d{4}\s*年\s*)?\d{1,2}\s*月\s*\d{1,2}\s*日\s*[，,、:：]?\s*", "", text, count=1)
...
def build_clean_hook(hook, words):
    ...
    # 确定性兜底（2026-09-14）：快闪不以具体日期开头（用户明确要求）
    marked = _strip_leading_date(marked)
...
def build_clean_story_cn(story_cn, words):
    ...
    # 确定性兜底（2026-09-14）：串讲不以具体日期开头（用户明确要求）
    marked = _strip_leading_date(marked)
    return marked, count
...
Then output these three objects:
  ...
  story: {{
    ...
    "cn": "中文热点综述（『热点串讲』板块主体），独立成篇，严格约 350 字（320-380 字为佳，不足 310 或超 400 不合格）。结构：①导语 1 句约 50 字总起今日财经/职场主线（**导语不得以具体日期开头，如『2026年9月14日』；直接切入主线**）；②主体 3 段，每段 80-110 字，分别展开一条具体主线（如某市场/资产动向、某行业或公司进展、职场/科技趋势），并把今日 8 个词自然写入叙述（中文显示即可，如『激增』『部署』『权衡』『韧性』『颠覆性的』）；③收束 1 句约 40 字收尾。这是中文原创综述，不是英文的翻译，不要逐词对应英文。\n\n重要：正文结尾不要写『总体看/总体而言』之类的总结句——串讲本身就是叙事，自然收束即可；统一的总结由 preview.impact 承担，不要在这里重复。
...
FINAL FORMAT RULE: preview.hook 与 story.cn 均不得以具体日期（如『2026年9月14日』）开头；preview.hook 必须是通顺的一句话、把今天这批词尽量都自然融入叙事，不要以『总体看/总体而言』这类总结句收尾（统一总结由 preview.impact 承担），禁止句尾清单式罗列；不要写任何 [..|..] 标记（自动添加）。story.cn 必须严格约 350 字（320-380 字为佳，不足 310 或超 400 不合格），且结尾不要写『总体看/总体而言』总结句（总结由 impact 承担）。
